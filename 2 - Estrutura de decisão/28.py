"""O Hipermercado Tabajara está com uma promoção de carnes que é imperdível. Confira:
                      Até 5 Kg           Acima de 5 Kg
File Duplo      R$ 4,90 por Kg          R$ 5,80 por Kg
Alcatra         R$ 5,90 por Kg          R$ 6,80 por Kg
Picanha         R$ 6,90 por Kg          R$ 7,80 por Kg
Para atender a todos os clientes, cada cliente poderá levar apenas um dos tipos de carne da promoção,
porém não há limites para a quantidade de carne por cliente. Se compra for feita no cartão Tabajara 
o cliente receberá ainda um desconto de 5% sobre o total da compra. Escreva um programa que peça o 
tipo e a quantidade de carne comprada pelo usuário e gere um cupom fiscal, contendo as informações da compra: 
tipo e quantidade de carne, preço total, tipo de pagamento, valor do desconto e valor a pagar.
"""

print('File Duplo[F] - Alcatra[A] - Picanha[P]: ')
tipo = input('Qual carne: ').upper()
quantidade = float(input('Quantos Kg: '))
cartao = int(input('No cartão Tabajara?[0-Não,1-Sim]:'))

if tipo == 'F':
    if quantidade <= 5 and quantidade > 0:
        x = 4.90
        valor = quantidade * x
    else:
        x = 5.80
        valor = quantidade * x
    tipo = 'Filé Duplo'
elif tipo == 'A':
    if quantidade <= 5:
        x = 5.90
        valor = quantidade * x
    else:
        x = 6.80
        valor = quantidade * x
    tipo = 'Alcatra'
elif tipo == 'P':
    if quantidade <= 5:
        x = 6.90
        valor = quantidade * x
    else:
        x = 7.80
        valor = quantidade * x
    tipo = 'Picanha'
else:
    print('Digite os dados corretamente')

if cartao == 1:
    valor = valor - ((valor /100)*5)
    pagamento = 'Cartão Tabajara'
    desconto = int(5)
else:
    pagamento = 'Dinheiro'
    desconto = int(0)

# valor de X é o preço por kilo
print('\n---- CUPOM FISCAL ----\n')
print(f'tipo: {tipo}\nquantidade: {quantidade}Kg\nvalor total: R${quantidade*x}\nPagamento: {pagamento}\nDesconto: {desconto}\nPreço: R${valor}')